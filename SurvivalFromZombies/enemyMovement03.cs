...
public class enemyMovement03 : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        audioSource_Enemy = this.transform.Find("chr_army3Child").GetComponent<AudioSource>();
    ...
    private void FixedUpdate()
    {
        if (GPManager.stageState == StageState.Stage03)
        {
            ...
            {
                if (this.gameObject.tag == "Stage03Enemy")
                {
                    if (this.transform.Find("chr_army3Child").GetComponent<MeshRenderer>().enabled)
                    {
                        this.transform.Find("chr_army3Child").GetComponent<MeshRenderer>().enabled = false;

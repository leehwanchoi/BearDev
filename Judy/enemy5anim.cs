...
public class enemy5anim : MonoBehaviour
{
    float speed;
    ...
    void Update()
    {
        ...
        if (Vector3.Distance(playerCoor.position, transform.position) <= 5f && !player.isDie && !isDie && !isDamage)
            distanceCheck();
    ...
    void distanceCheck()
    {
        ...
        if (Vector3.Distance(playerCoor.position, transform.position) >= 0.75f)
        {
            speed = 0.5f;
            MoveUpdate();
        }
